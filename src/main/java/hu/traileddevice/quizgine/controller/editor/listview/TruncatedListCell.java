...
public class TruncatedListCell extends ListCell<CellDisplayable> {
    @Override
    protected void updateItem(CellDisplayable item, boolean empty) {
        super.updateItem(item, empty);

        if (empty || item == null || item.getCellContent() == null) {
            setText(null);
        } else {
            String noLineBreak = item.getCellContent().replaceAll("\\R+", "");
